...
class TablaProduccionFecha extends Model
{
    ...
    public function obtenerUltimasFechasIDs($fechaActual)
    {
        return $this->select('Fecha_de_Produccion, idTabla_Produccion')
            ->where('Fecha_de_Produccion <=', $fechaActual)
            ->orderBy('Fecha_de_Produccion', 'DESC')
            ->limit(11)
            ->findAll();
    }
}
